...
class Date extends CComponent{
    ...
    public function timeAgo($beginDate){
    	$tag = $unit = ""; 
        $type = "success";   	
		$value = intval((strtotime('now') -strtotime($beginDate)) /60 );    		
		if($value < 60){    		
			$unit = " minuto";	 
			$tag = "Hace ";            
		}else{
			...
			$unit = " hora";
			$tag = "Hace más de ";
            if($value > 5)  $type = "warning";			
            if($value > 10) $type = "important";          
		}
		if($value >1)
			$unit .="s";
    	  	    	
    	return array('text' => $tag . $value .$unit, 'type' => $type);
    	     	
    }

    public function duration($beginDate, $endDate){    

        $value = intval((strtotime($endDate) -strtotime($beginDate)) /60 );            
